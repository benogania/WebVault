  ...
  return (
    <TouchableOpacity onPress={onPress} onLongPress={onLongPress} activeOpacity={0.8} className="w-[31%] items-center mb-4">
      <View className="w-full h-28 bg-slate-800 rounded-xl border border-slate-700/60 p-3 justify-between items-center shadow-lg">
        ...
        <Text className="text-white text-xs font-bold text-center mt-2 w-full" numberOfLines={1}>{folder.name}</Text>
      </View>
    </TouchableOpacity>
  );
};
...
export default function HomeScreen({ navigation }) {
  ...
  // FIXED: Now properly routes to our custom viewers instead of Expo WebBrowser
  const openSite = (item) => {
    if (item.isDownloaded && item.localUri) {
      navigation.navigate("OfflineViewerScreen", { site: item });
    } else {
      // Direct to our new Live Web View
      navigation.navigate("WebViewerScreen", { site: item });
    }
  };
  ...
  return (
    <View className="flex-1 bg-slate-900">
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
            ...
            <View className="flex-row flex-wrap px-4 mb-2 justify-start" style={{ columnGap: "3.5%" }}>
              {filteredPinned.map((item) => (
                <VaultCard 
                  ...
                  {...item} 
                  hidePinIcon={true} 
                  onPress={() => openSite(item)} // FIXED: Passing item, not item.url
        ...
        <View className="flex-row flex-wrap px-4 mb-2 justify-start" style={{ columnGap: "3.5%" }}>
          {filteredFolders.map((folder) => (
            <FolderCard key={folder.id} folder={folder} sites={vaultSites} onPress={() => setOpenedFolder(folder)} onLongPress={() => setSelectedFolderActions(folder)} />
          ))}
          ...
          {filteredVault.length === 0 && filteredFolders.length === 0 && (
            <View className="w-full py-10 items-center">
              <Text className="text-slate-500 text-center px-10">{searchQuery ? "No sites or folders match your search." : "Your vault is empty."}</Text>
            </View>
          )}
        </View>
      </ScrollView>
              ...
              {currentFolderSites.length === 0 ? (
                ...
                currentFolderSites.map((item) => (
                  <VaultCard 
                    ...
                    {...item} 
                    onPress={() => openSite(item)} // FIXED
                    onLongPress={() => { setOpenedFolder(null); setTimeout(() => setSelectedSite(item), 300); }} 
                  />
                ))
              )}
            </View>
            ...
            <TouchableOpacity
              onPress={() => {
                Alert.alert("Delete Folder", `Are you sure you want to delete ${selectedFolderActions?.name}?`, [
                  ...
                  { text: "Delete", style: "destructive", onPress: () => { deleteFolder(selectedFolderActions.id); setSelectedFolderActions(null); if (Platform.OS === "android") ToastAndroid.show("Folder deleted", ToastAndroid.SHORT); } }
                ]);
              }}
      ...
      <SiteActionModal visible={!!selectedSite} site={selectedSite} onClose={() => setSelectedSite(null)} onTogglePin={togglePin} onDelete={deleteSite} onEdit={handleEdit} />
    </View>
  );
}